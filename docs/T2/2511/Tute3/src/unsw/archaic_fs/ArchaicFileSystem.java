...
public class ArchaicFileSystem {
    private List<Inode> inodeLookup = new ArrayList<>();
    private List<Inode> currentPath = new ArrayList<>();
        ...
        for (int i = 0; i < pathComponents.length; i++) {
            ...
            if (inode == null) {
                if (createParentDirectories || i == pathComponents.length - 1) {
                    ...
                    throw new FileNotFoundException(formPathForInode(current) + "/" + pathComponents[i]);
                }
            } else if (ignoreIfExists || i != pathComponents.length - 1) {
                current = inode;
            } else {
                throw new FileAlreadyExistsException(formPathForInode(current) + "/" + pathComponents[i]);
            }
        }
    }
    ...
    public void cd(String path) throws IOException {
        ...
        for (String component : pathComponents) {
            ...
            if (inode == null) {
                throw new NoSuchFileException(formPathForInode(current) + "/" + component);
            } else {
                current = inode;
            }
        }
    ...
    public String readFromFile(String path) throws IOException {
        ...
        if (inode != null) {
            ...
            // doesn't exist
            throw new FileNotFoundException(formPathForInode(current) + "/" + filename);
        }
    }
    ...
    public void writeToFile(String path, String content, EnumSet<FileWriteOptions> opts) throws IOException {
        ...
        if (opts.contains(FileWriteOptions.CREATE)) {
            if (inode == null) {
                ...
                throw new FileAlreadyExistsException(formPathForInode(current) + "/" + filename);
            }
